Feature: TC07_Shop for RoundTrip  in TS, Price, Book, Ticketing, AMOA, SeatMap & AMF in UAPI and Exchange(ADCOL) on SmartPoint
...
And RoundTrip segments
	...
	Examples:
	      | PCC  | TripType  | FromTo  		 | DepartureDates   | PaxType 			| ContentSource |CabinPreferenceType | Cabin  | AccessGroup                         | returnBrandedFaresInd |
				| 31RB | RoundTrip | AMS-LHR-AMS | 185,190 						| 1-ADT 	| GDS 					| Permitted					 |Economy|A12D96C8-EB29-4443-A27E-18A72C183E87 ||
...
Scenario Outline: Post ticketing - Add collect
    ...
    And Close SmartPoint
      ...
      | LON     | 31RB  | VoluntaryChange |   1-2	   |    C   |Edreamz|
